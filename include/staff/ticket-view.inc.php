...
if ($conn->connect_error) {
  die("Ha fallado la conexión con la base de Datos :( : " . $conn->connect_error);
    ...
    $sql = "SELECT * FROM kardex WHERE idticket =  $ticketid";
...
<!-- Botones -->
<div class="btnPrestamoF" id = "btnprestamo">
<?php if($estado == 0){
    echo("<button class='botonPrestar' onclick='prestar()'> Realizar Préstamo</button>");
} 
?>
</div>
<div class="btnRegresarF" id = "btnregreso">
<?php if($estado == 1){
    echo("<button class='botonRegresar' onclick='regresar()'> Regresar Herramienta</button>");
...
<style>
    ...
    .botonPrestar {
        ...
        font-weight: 300;
        color: #ffffff;
        text-shadow: 0 0.04em 0.04em #006784;
        text-align: center;
        transition: all 0.2s;
    }

    .botonPrestar:hover {
        border: 3px solid #006682;
        border-color: #006682;
        cursor: pointer;
    }
...
<?php $idticket = $ticket->getId();?>

<div>
<iframe id="iframe" style= "border: white solid 5px; display: none"
    title="Prestar herramienta"
    width="900"
    height="400"
    src = "http://172.16.50.8:8080/ticket?ticket=<?php echo($idticket) ?>"
>
</iframe>
</div>
...
<div>
<iframe id="iframeregreso" style= "border: white solid 5px; display: none"
    title="Prestar herramienta"
    width="900"
    height="400"
    src = "http://172.16.50.8:8080/regreso?ticket=<?php echo($idticket) ?>"
>
</iframe>
</div>

<script>
var spanContent = document.getElementById("info");
if(spanContent.innerHTML.includes("Valeria")){
        console.log("Bienvenida Valeria");
    }
    else{
        console.log("Bienvenido Richard");
        document.getElementById('btnprestamo').style.display = 'none';
        document.getElementById('btnregreso').style.display = 'none';
...
function regresar(){
    document.getElementById('iframeregreso').style.display = 'block';
    console.log("It works");
}
</script>
